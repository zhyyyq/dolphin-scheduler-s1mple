...
const DiffViewer: React.FC<{ commit: Commit; workflowUuid: string }> = ({
  commit,
  workflowUuid,
}) => {
    ...
    const fetchDiff = async () => {
      try {
        const response = await api.get<{ diff: string }>(
          `/api/workflow/${workflowUuid}/commit/${commit.hash}`
        );
...
const WorkflowHistoryPage: React.FC<{ workflow_uuid: string }> = ({
  workflow_uuid,
}) => {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      const data = await api.get<Commit[]>(
        `/api/workflow/${workflow_uuid}/history`
      );
      setHistory(data);
    } catch (err) {
      const errorMessage =
  ...
  return (
    <Modal open={true} width="80%" title="工作流变更记录" footer={null} onCancel={() => {
      const event = new CustomEvent('check_workflow_changes_end', {
        detail: "user canceled"
      });
      document.querySelector("scheduler-workflow-history")?.dispatchEvent(event);
    }}>
      <div style={{ padding: "24px", background: "#fff", borderRadius: "8px" }}>
        <Title level={2} style={{ marginBottom: "24px" }}>
          History for {workflow_uuid}
        </Title>
        <Table
          columns={columns}
          dataSource={history}
          ...
          expandable={{
            expandedRowRender: (record) => (
              <DiffViewer commit={record} workflowUuid={workflow_uuid!} />
            ),
            rowExpandable: () => true,
          }}
        />
