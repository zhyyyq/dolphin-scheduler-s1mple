            ...
            >
              <AntApp style={{ flex: 1 }}>
                <WorkflowInstanceDetailPage
                  projectCode={this.getAttribute("projectCode") || ""}
        ...
        <Provider store={store}>
          <ConfigProvider>
            ...
            >
              <AntApp style={{ flex: 1 }}>
                <PythonEditorPage
                  functionId={this.getAttribute("functionId") || ""}
                />
              </AntApp>
            </ConfigProvider>
          </ConfigProvider>
        ...
        <Provider store={store}>
          <ConfigProvider>
            ...
            >
              <AntApp style={{ flex: 1 }}>
                <WorkflowHistoryPage
                  workflow_uuid={this.getAttribute("workflow_id") || ""}
                />
              </AntApp>
            </ConfigProvider>
          </ConfigProvider>
            ...
            >
              <AntApp style={{ flex: 1 }}>
                <SchedulerEditor
                  ...
                  projectCode={projectCode ? parseInt(projectCode) : null}
...
export class SchedulerSDK {
  ...
  init() {
    ...
    customElements.define(
      SchedulerEditorComponent.component_name,
      SchedulerEditorComponent
    ); // 定义自定义元素名称和类名关联起来
    customElements.define(
      SchedulerWelcomeComponent.component_name,
      SchedulerWelcomeComponent
    ); // 定义自定义元素名称和类名关联起来
    customElements.define(
      SchedulerDashboardComponent.component_name,
      SchedulerDashboardComponent
    ); // 定义自定义元素名称和类名关联起来
    customElements.define(
      SchedulerMaintainComponent.component_name,
      SchedulerMaintainComponent
    );
    customElements.define(
      SchedulerManageComponent.component_name,
      SchedulerManageComponent
    );
    customElements.define(
      SchedulerFunctionManageComponent.component_name,
      SchedulerFunctionManageComponent
    );
    customElements.define(
      SchedulerWorkflowHistoryComponent.component_name,
      SchedulerWorkflowHistoryComponent
    );
    customElements.define(
      SchedulerEditFunctionComponent.component_name,
      SchedulerEditFunctionComponent
    );
    customElements.define(
      SchedulerInstanceDetailComponent.component_name,
      SchedulerInstanceDetailComponent
    );
  }
  async create_or_modify_workflow(
    workflow_id?: string,
    mount_ref?: HTMLElement | null,
    projectCode?: string,
    projectName?: string
  ) {
    // 低代码形式
  ...
  async get_workflow_info(workflow_id: number) {
    // 获取工作流配置
    const response = await api.get<WorkflowData>(
